...
My goal with this site was to replicate a real world application, and production environment/attmosphere to as much of an extent as I could (whether this be in quality or in other features such as CI/CD or testing). When I was looking for a way in which I could build a CI/CD pipeline for this project I weighed up my options between Jenkins, GitLab and Github Actions. As I was already using GitHub at the time as my source control manager, my choice gravitated towards either Github Actions and Jenkins, but GitHub actions seemed a bit more convenient in it's setup and use (there is no need to set up or maintain servers - GitHub Actions provides that out of the box). This meant that I ended up chosing GitHub Actions and built out my workflow in a ci.yml file (defining the workflow in yml being one of the benefits of GitHub Actions as it is simple in comparison to Jenkins which uses Groovy to define it's pipelines).
